...
import { ProductView, Layout, Loading, Retry } from "../components";
    ...
    try {
      ...
      sanityClient.fetch(productQuery).then((productQueryRes) => {
        if (productQueryRes.length < 1) {
          ...
          setPageContent(<ProductView _id={params.productId} categoryName={productQueryRes[0].category.name} imageUrls={imageUrls} productId={productQueryRes[0].productId}/>)
        }
      })
    } catch (error) {
      console.error("error");
