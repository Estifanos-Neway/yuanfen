...
export default function ProductDetail() {
  ...
  useEffect(() => {
    try {
      const productQuery = `*[_id == '${params.productId}']{category->,primaryImage{asset->{url}}, secondaryImages[]{asset->{url}}, productId, price}`;
      sanityClient.fetch(productQuery).then((productQueryRes) => {
        if (productQueryRes.length < 1) {
          ...
          setPageContent(<ProductDetailView _id={params.productId} categoryName={productQueryRes[0].category.name} imageUrls={imageUrls} productId={productQueryRes[0].productId} price={productQueryRes[0].price} />)
        }
      })
    } catch (error) {
      console.error("error");
